...
void msync_init_shm(void)
{
    ...
    kr = mach_vm_allocate( mach_task_self(), (mach_vm_address_t *)&shm_tid_map, shm_tid_size, VM_FLAGS_ANYWHERE );

    if (kr != KERN_SUCCESS)
    {
        fprintf( stderr, "msync: error: mach_vm_allocate failed with %d: %s\n", kr, mach_error_string( kr ) );
...
static void *get_shm( unsigned int idx )
{
    ...
    if (!shm_addrs[entry])
    {
        ...
        kr = mach_vm_allocate( mach_task_self(), (mach_vm_address_t *)&address, (mach_vm_size_t)pagesize, VM_FLAGS_ANYWHERE );
        MACH_CHECK_ERROR( kr, "mach_vm_allocate" );
        if (kr != KERN_SUCCESS)
            fatal_error( "msync: Could not allocate page.\n" );
