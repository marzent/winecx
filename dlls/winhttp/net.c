...
void netconn_close( struct netconn *conn )
{
    if (conn->secure)
    {
        ...
        heap_free(conn->ssl_read_buf);
        heap_free(conn->ssl_write_buf);
                              ...
                              BOOL check_revocation )
{
    ...
    while(status == SEC_I_CONTINUE_NEEDED || status == SEC_E_INCOMPLETE_MESSAGE) {
        ...
        if(status == SEC_E_OK) {
            ...
            conn->ssl_read_buf = heap_alloc(conn->ssl_sizes.cbHeader + conn->ssl_sizes.cbMaximumMessage + conn->ssl_sizes.cbTrailer);
             if(!conn->ssl_read_buf) {
                 res = ERROR_OUTOFMEMORY;
                 break;
             }
            conn->ssl_write_buf = heap_alloc(conn->ssl_sizes.cbHeader + conn->ssl_sizes.cbMaximumMessage + conn->ssl_sizes.cbTrailer);
